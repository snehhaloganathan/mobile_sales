                        ...
                        <div class="row align-items-end">
                            <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    ...
                                    <input type="date" class="form-control form-control-sm rounded-0" name="fdate"  value="<?= $fdate ?>" required="required">
                                    <label for="date" class="control-label">To:</label>
                                    <input type="date" class="form-control form-control-sm rounded-0" name="tdate"  value="<?= $tdate ?>" required="required">
                                </div>
                            </div>
                            <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                                <div class="form-group">
                                    <button class="btn btn-sm btn-flat btn-dark bg-gradient-dark" name=submit><i class="fa fa-filter"></i> Filter</button>
                                </div>
                            </div>
                        </div>
                    </form>
                ...
                <div class="container-fluid" id="printout">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            $g_total = 0;
                            $i = 1;
                            $requests = $conn->query("SELECT oi.*,o.code, concat(p.brand, ' ', p.name) as product FROM `order_items` oi inner join order_list o on oi.order_id = o.id inner join product_list p on oi.product_id = p.id where date(o.date_created) between '$fdate' and '$tdate' order by abs(unix_timestamp(o.date_created)) asc ");
